...
import io.trino.gateway.GatewayResponseHandler.ProxyResponse;
        ...
        implements ResponseHandler<ProxyResponse, RuntimeException>
{
    ...
    {
        throw new GatewayException("Request to remote Presto server failed", exception);
    }
    ...
    public ProxyResponse handle(Request request, Response response)
    {
        ...
        if (response.getStatusCode() != OK.code()) {
            throw new GatewayException(format("Bad status code from remote Presto server: %s: %s", response.getStatusCode(), readBody(response)));
        }

        String contentType = response.getHeader(CONTENT_TYPE);
        if (contentType == null) {
            throw new GatewayException("No Content-Type set in response from remote Presto server");
        }
        if (!MediaType.parse(contentType).is(MEDIA_TYPE_JSON)) {
            throw new GatewayException("Bad Content-Type from remote Presto server:" + contentType);
        }
        ...
        try {
            ...
            throw new GatewayException("Failed reading response from remote Presto server", e);
        }
    }
